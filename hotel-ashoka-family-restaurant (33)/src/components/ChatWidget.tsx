...
export default function ChatWidget() {
  ...
  const [language, setLanguage] = useState<'English' | 'Marathi' | null>(() => {
    return localStorage.getItem('chat_language') as "English" | "Marathi" | null;
  });
  ...
  const [clientProfile, setClientProfile] = useState<Record<string, string>>(() => {
    const saved = localStorage.getItem('chat_client_profile');
    if (saved) {
      try { return JSON.parse(saved); } catch (e) {}
    }
    return {};
  ...
  useEffect(() => {
    localStorage.setItem('chat_client_profile', JSON.stringify(clientProfile));
  }, [clientProfile]);
  ...
  const processMessage = async (text: string) => {
    ...
    // Intercept Contact
    if (lowerText.includes('contact') || lowerText.includes('phone') || lowerText.includes('call') || lowerText.includes('whatsapp') || lowerText.includes('number') || lowerText.includes('संपर्क') || lowerText.includes('फोन') || lowerText.includes('नंबर')) {
      const botMsg: Message = {
        id: (Date.now() + 1).toString(),
        text: language === 'Marathi'
    ...
    try {
      ...
      const currentProfile = JSON.parse(localStorage.getItem('chat_client_profile') || '{}');
      let memoryInstruction = "";
      if (Object.keys(currentProfile).length > 0) {
        memoryInstruction = `\n\nKNOWN CLIENT DETAILS (Stored in Memory):\n${JSON.stringify(currentProfile)}\nUse these details to personalize your response and greet the user by name if known.`;
      }
      ...
      - Location/Address: We are located on Osmanabad Rd, near the old bus stand, Tuljapur, Maharashtra 413601.
      - Contact Details: Phone & WhatsApp number is +91 99227 72200. Instagram: @hotel_ashoka_tuljapur.
      ...
      MEMORY INSTRUCTION:
      If the user provides any personal details natively (like their name, contact info, dietary preferences), you MUST output a special JSON string at the VERY END of your response to save it to memory. Format it exactly like this:
      [SAVE_MEMORY: {"name": "John Doe", "preferences": "likes window seat"}]
      Provide your normal conversational response before this tag. Provide as much detail as you learned. Do not mention that you are using this tag. This tag will be intercepted by the system.
      ${memoryInstruction}`;

      const historyContext = messages
        .filter((m) => m.id !== '1' && m.sender !== 'error')
        .map((m) => ({
      ...
      const contents = [...historyContext, { role: 'user', parts: [{ text }] }];
      ...
      for await (const chunk of responseStream) {
        if (chunk.text) {
          ...
          const memoryMatch = accumulatedText.match(/\[SAVE_MEMORY:\s*(\{.*?\})\s*\]/);
          if (memoryMatch) {
            try {
               const newProfile = JSON.parse(memoryMatch[1]);
               setClientProfile(prev => ({ ...prev, ...newProfile }));
            } catch(e) {
               console.error("Error parsing memory tag:", e);
            }
            displayText = accumulatedText.replace(/\[SAVE_MEMORY:\s*(\{.*?\})\s*\]/, '').trim();
          }
...
${Object.keys(JSON.parse(localStorage.getItem('chat_client_profile') || '{}')).length > 0 ? `KNOWN CLIENT DETAILS (Stored in Memory):\n${JSON.stringify(JSON.parse(localStorage.getItem('chat_client_profile') || '{}'))}\nUse these details to personalize your response and greet the user by name if known.` : ''}
...
- Menu questions: We serve 100% Pure Vegetarian Indian cuisine. Popular items include Maharashtrian thali, soups, breads, Lasuni Methi, Manchow Soup Veg.
- Location/Address: We are located on Osmanabad Rd, near the old bus stand, Tuljapur, Maharashtra 413601.
- Contact Details: Phone & WhatsApp number is +91 99227 72200.
- Timings: We are open from 11 AM to 11:30 PM.
            ...
            <div className="bg-brand-dark text-white p-4 flex justify-between items-center">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 bg-brand-cream rounded-full flex items-center justify-center overflow-hidden border border-brand-yellow/30">
                  <img 
                    src="https://lh3.googleusercontent.com/d/1oyU_50Ji7-vqhHhIZMyojkPPXQR2CwuT" 
